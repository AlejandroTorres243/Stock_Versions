...
public class ClientController {
    ...
    @Autowired
    private ClientRepositorio personRepository;

    @GetMapping
    private Flux<Client> getAllPersons() {
    ...
    @PostMapping("")
    private Mono<ResponseEntity<Client>> addPerson(@Valid @RequestBody Client person) {
        return personRepository.save(person)
                .map(ResponseEntity::ok)
                .defaultIfEmpty(ResponseEntity.notFound().build());
    }
    @GetMapping("/{id}")
    public Mono<Client> getPersonById(@PathVariable(value = "id") String id) {
