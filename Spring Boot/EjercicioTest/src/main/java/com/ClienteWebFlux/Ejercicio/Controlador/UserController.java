package com.ClienteWebFlux.Ejercicio.Controlador;

import com.ClienteWebFlux.Ejercicio.Cliente.*;
import com.ClienteWebFlux.Ejercicio.Repositorio.*;
...
public class UserController {
    ...
    @Autowired
    private UserRepositorio personRepository;
    ...
    @PostMapping("")
    private Mono<ResponseEntity<User>> addPerson(@Valid @RequestBody User person) {
        return personRepository.save(person)
                .map(ResponseEntity::ok)
                .defaultIfEmpty(ResponseEntity.notFound().build());
    }
    @GetMapping("/{id}")
    public Mono<User> getPersonById(@PathVariable(value = "id") String id) {
